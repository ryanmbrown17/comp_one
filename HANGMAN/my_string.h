...
//object
//postcondition: The memory used for MY_STRING object has been reclaimed by
//te system and the handle referred to by the pointer phMy_string has been
//set to NULL
...
//with the same values as the given c-string.  The capacity of the string
...
//copy of the address of the opaque object will be returnde on success and NULL
//on failure
MY_STRING my_string_init_c_string(const char* c_string);

//Precondition: hMy_string is the handle of a valid My_string object
//Postcondition: return a copy of the integer value of the object's capacity
int my_string_get_capacity(MY_STRING hMystring);

//Precondition: hMy_string is the handle of a valid My_string Object
//Postconition: Returns a copy of the integer value of the objects size.
int my_string_get_size(MY_STRING hMy_string);

//Precondition: hLeft_string and hRight_string are valid My_string objects
//Postcondition: returns an integer less than zero if the string
//represented by hLeft_string is lexicographically smaller than hRight_string. If
//one string is a prefix of the other string then the shorter string is 
//considered to be the smaller one. (app is less than apple).  Returns 0 if the
//strings are the same and returns a number greater than zero if the string 
//represented by hLeft_string is bigger than hRight_string
int my_string_compare(MY_STRING hLeft_string, MY_STRING hRight_string);


//Precondition: hMystring is the handle to a valid My_string object.
//Post Condition hMy_string will be the handle of a string object that contains
...
//	and included) will be added to the string until a stoping condition is met.
//	The capacity of the string will continue to grow as needed until all the
//	characters are stored.
//	3) A stopping condition is met if we read a whitespace character after
//	we have read at least one non-whitespace character or if we reach the end of the file
//Function will return SUCCESS if a non-empty string is read successfully.
//and failure otherwise.  Remeber that the incoming string may already contain some
...
//Function will return SUCCESS if it sucessfully writes the string and FAILURE otherwise.
